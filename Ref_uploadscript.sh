...
for x in *gff3 ; do m=$(echo $x | sed 's/.gff3//g'); n=$(egrep ^##sequence-region.*_01 clean_gff/$x | perl -nle 'if(/(\S+)_01_(v\d+)/){ print "$1\t$2"} elsif(/(\S+)_01/){ print $1 } ' ); echo -e "$m\t$n"; done > ChromosomeFile.txt # generate Chromosom file
